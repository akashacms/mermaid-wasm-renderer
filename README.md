...
git -C vendor/mermaid-rs-renderer checkout 2f993bd   # known-good commit for the patch (v0.3.1)
...
git -C ../mermaid-rs-renderer checkout 2f993bd
...
The `checkout 2f993bd` pin is the commit the patch was generated against; upstream moves quickly, so the patch may need rebasing on newer commits (`git apply --3way` usually resolves it).
